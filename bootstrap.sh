...
sudo pip install --allow-external https://cdn.mysql.com/Downloads/Connector-Python/mysql-connector-python-2.1.14.tar.gz
...
sudo pip install --allow-external libffi-dev libffi-dev requests
...
chmod +x *.run
sudo n y y y | ./xampp-linux-x64-5.6.28-1-installer.run
